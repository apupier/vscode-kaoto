...
export class VSCodeKaotoEditorChannelApi extends DefaultVsCodeKieEditorChannelApiImpl implements KaotoEditorChannelApi {
  ...
  async getResourceContent(relativePath: string): Promise<string | undefined> {
    const classpathRoot: string = this.findClasspathRoot();
    try {
  ...
  private findClasspathRoot() {
    const camelRoutePath = this.currentEditedDocument.uri.fsPath;
    ...
    if (indexOfClasspath !== -1) {
      ...
      // In non Maven based project, we consider the folder at same level than Camel route to bethe classpath route
      classpathRoot = path.dirname(this.currentEditedDocument.uri.fsPath);
    }
